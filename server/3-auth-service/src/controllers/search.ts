...
import { IPaginateProps, ISearchResult } from '@auth/utils';
...
export async function singleGigById(req: Request, res: Response): Promise<void> {
  const gig = await gigById('gigs', req.params.id);
  res.status(StatusCodes.OK).json({ message: 'Search gigs results', gig });
}
