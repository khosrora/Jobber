import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@users/utils';
...
const seed = async (req: Request, res: Response): Promise<void> => {
  ...
  for (let i = 0; i < buyers.length; i++) {
    ...
    if (checkIfSellerExist) {
      throw new BadRequestError('Seller already exist', 'SellerSedd seller() method error');
    }
    ...
    const seller: ISellerDocument = {
      ...
      experience: randomExpriences(parseInt(faker.commerce.price({ min: 1, max: 5, dec: 0 }))),
      education: randomEducation(parseInt(faker.commerce.price({ min: 1, max: 5, dec: 0 }))),
      socialLinks: ['https://kickchatapp.com', 'https://facebook.com'],
      certificates: [
        {
...
const randomExpriences = (count: number): IExperience[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const experience = {
      ...
      startDate: `${faker.date.month()} ${randomStartYear[floor(random(0.9) * randomEndYear.length)]} `,
...
const randomEducation = (count: number): IEducation[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const education = {
      ...
      title: faker.person.jobTitle(),
      major: `${faker.person.jobTitle()} ${faker.person.jobDescriptor()}`,
