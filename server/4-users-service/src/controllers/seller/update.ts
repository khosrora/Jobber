import { BadRequestError, ISellerDocument } from '@users/utils';
...
const update = async (req: Request, res: Response): Promise<void> => {
  ...
  const createdSeller: ISellerDocument = await updateSeller(req.params.sellerId, seller);
  res.status(StatusCodes.OK).json({ message: 'Seller created successfully.', seller: createdSeller });
};
