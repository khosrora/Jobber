...
import { IBuyerDocument, ISellerDocument } from '@users/utils';
import { createBuyer, updateBuyerPurchasedGigsProps } from '@users/services/buyer.service';
import {
  updateSellerOnGoingJobsProp,
  updateSellerCompletedJobsProp,
  updateTotalGigsCount,
  updateSellerCancellJobsProp,
  updateSellerReview,
  getRandomSellers
} from '@users/services/seller.service';
import { publishDirectMessage } from './user.producer';

const log: Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'user service consumer', 'debug');
  ...
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'auth') {
        ...
        await updateBuyerPurchasedGigsProps(buyerId, purchasedGigs, type);
      }
      channel.ack(msg!);
    });
  } catch (error) {
  ...
  try {
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'create-order') {
        ...
        await updateSellerCancellJobsProp(sellerId);
      }
      channel.ack(msg!);
    });
  } catch (error) {
...
const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'jobber-review-update';
    ...
    log.log('error', 'UsersService consumeReviewFanoutMessages() method Error :', error);
  }
};
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'getSellers') {
        ...
        await publishDirectMessage(
          ...
          'jobber-seed-gig',
          'recive-sellers',
          JSON.stringify({ type: 'reciveSellers', sellers, count }),
          'message sent to gig service'
        );
      }
      channel.ack(msg!);
