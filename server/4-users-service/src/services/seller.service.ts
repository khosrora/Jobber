...
import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@users/utils';
...
const getRandomSellers = async (count: number): Promise<ISellerDocument[]> => {
  const buyer: ISellerDocument[] = await SellerModel.aggregate([
    {
      $sample: {
        size: count
      }
...
const updateSellerCancellJobsProp = async (sellerId: string): Promise<void> => {
